// kit-post: the architecture kit's post step around tools/meshy's meshy-post.
//
...
export async function kitPost(input, output, job) {
	...
	try {
		...
		for (const mat of doc.getRoot().listMaterials()) {
			// architecture never glows: Meshy's refine sometimes ships a faint emissive map
		...
		return { ...report, output, clamped: moved, graded, bytesOut: fs.statSync(output).size };
	} finally {
